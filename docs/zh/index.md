...
!!! warning "关于实验与规模"
    这些实验会真实创建 Confidential VM、Confidential GKE、A3 与 A4 GPU 资源，其中数个还会刻意同时跑多套配置。这不是铺张：本课程中几乎每一个论断都是两种环境之间的*差值*——机密与非机密、Hopper 与 Blackwell、这个验证方与那个验证方——而差值是无法用一个实例测出来的。凡是实验要求配对照组的地方，就把对照组开出来；用玩具规模跑出来的数字不是"不够精确"，而是朝着好看的方向错的。每个实验都会标注其规模，并说明其命令是在真实项目上执行过的，还是从厂商文档转录、标记为**未验证**的。真正的约束是容量而非费用——机密 GPU 机型受地域限制，安排实验前请先确认可用性。
