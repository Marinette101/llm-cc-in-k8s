...
**目标**：在 TEE 内部服务一个真的会有人付钱的模型——不是玩具——权重密钥只在 CPU + GPU + 镜像的复合证明通过时才释放，TLS 终结在 §5 所要求的位置上，客户端会拒绝与未经证明的服务端对话，并产出一份你能在容量评审上站得住脚的、实测的冷启动预算。

**规模**：模块 4 实验里那个机密 8×B200 节点、一个 70B 级模型、模块 3 实验里那个由提供方运营的验证方，以及一个专门立起来的托管 L7 负载均衡器——立它纯粹是为了让你能*演示* §5.1 里那个失败，而不是断言它。请按真实尺寸跑。一个 3B 模型跑在单卡上，会给出一个差着约一个数量级的冷启动数字、一个小到显现不出 §6.2 效应的 KV 缓存，以及一个短到足以掩盖"整套架构正是为管理它而存在"的解密阶段。玩具规模跑出来的数字并不保守——它们是朝着好看的方向错的。**状态**：本实验由模块 3、4、5 实验中已核实的部件组合而成；组合本身作为设计练习给出。请逐条对照当前文档核实命令。

### 第 1 步 —— 加密一个真实的模型

```bash
# 生成 DEK、加密模型、用你控制的 KEK 包裹 DEK。
# 70B/bf16 下这是约 140 GB —— 这正是重点。这一步也要计时。
...
shred -u dek.bin   # 明文 DEK 绝不能在这一步之后继续存在
```

记下加密并上传 140 GB 的实际耗时。那个数字就是你的模型发布流水线，而各个团队往往要到第一次赶着 deadline 发模型更新时，才第一次发现它。

### 第 2 步 —— 构建推理镜像

容器应当依次：从 launcher socket 请求证明 token；把原始证据交给模块 3 E 部分里**你自己的验证方**而不是 Google 的；解包 DEK；把模型拉取并解密进 `tmpfs`；以 `tensor_parallel_size=8` 启动 vLLM；并暴露一个会返回自身证明 token 的端点，供客户端验证。

用 cosign 给镜像签名并记录 digest。
...
然后在你自己的验证方里、基于原始证据断言同样这几条，并让*那一条*成为真正闸控密钥的判定。Google 侧的策略就此降格为纵深防御，而不再是那个保证本身。

### 第 4 步 —— 部署，并给每一个阶段计时

给容器加埋点，在 §4.1 的每个冷启动里程碑记一个时间戳：启动完成、内存接受完成（它的量级你在模块 1 第 6 步已经测过）、证明完成、密钥释放、权重拉取、权重解密并加载进受保护 HBM、首 token 送出。**产出你这套配置的实际预算表。** 这份产物比本实验其余部分加起来都更有价值——它是每一场容量与定价对话都会要的那个数字。

至少跑五遍。这条路径上的冷启动有很长的尾巴，单次采样会让你对"真正决定热池规模的那个 p99"产生误判。

### 第 5 步 —— 先把入口的失败演出来，再修好它

§5.1 把"在 TEE 前面放一个托管 L7 负载均衡器"称为这套设计中最常见的静默失败。故意先建错，然后把它证明出来：

```bash
# §5.2 的方案 A —— 由 Google 终结 TLS 并持有私钥
...
从它送一个 prompt 进去，然后从负载均衡器自己的请求日志里把这个 prompt 读回来。**prompt 以明文形态存在于 Google 运营的基础设施里，而下游每一个机密机制都完好无损、且毫不相干。** 把那行日志截下来放进你的设计评审材料；它结束争论的速度比那张图快得多。

现在切到方案 B——L4 直通负载均衡器，RA-TLS 在 TEE 内部终结，公钥绑进 `REPORT_DATA`——确认同一个请求在进入 enclave 之前的任何一跳上都读不出任何东西。

### 第 6 步 —— 从客户端一侧验证

写一个客户端，在发送任何 prompt 之前：

1. 取回服务端的证明 token 与原始证据。
2. 对照你自己的验证方核验签名与各项 claim——包括 `cc_mode`、TCB 下界与镜像 digest。
3. 确认 RA-TLS 公钥哈希出现在 `REPORT_DATA` 里，从而确保它在跟*那个* enclave 说话，而不是一个中继。
4. **任何一项检查失败就拒绝发送任何东西。**

然后用三种方式打破它：部署一个 digest 不在白名单里的镜像；在中间放一个代理，转发一份来自另一台机器的合法证明；把 TCB 下界抬到整个机群之上。客户端应当三次都拒绝。这个拒绝就是 P3 在起作用，而第 3 条正是让它能扛住中继攻击的原因。

### 第 7 步 —— 把前缀缓存预言机认真测出来

在前缀缓存开启且跨租户共享的前提下，这是一次测量，而不是一次演示——所以就按测量的方式来做：

```python
# 受害"租户"先用一段有特征的长前缀把缓存暖上。
# 攻击者租户随后对猜测的前缀与一个对照分别测 TTFT。
for trial in range(200):
    ttft_hit  = time_ttft(shared_prefix + attacker_suffix)
    ttft_miss = time_ttft(random_prefix  + attacker_suffix)
```

把两个分布画出来。前缀足够长时，它们会清晰分离——意味着攻击者能以很高的置信度区分"别的租户发过这段文字"与"没人发过"，而全程没有读取任何一个内存字节。用准确率来报告它：单次计时采样分类正确的比例是多少？

然后把前缀缓存改为按租户隔离，重复实验，直到两个分布重叠为止。**你自己数字里的那个差异就是 §6.2**，也是全书中最有说服力的一次演示：一次机密性失败，完全可以不涉及读取任何内存。

### 第 8 步 —— 对着运行中的系统做一次对抗性评审

§9 列出了一个拥有主机 root 的内部人员仍然能做的事。在系统运行着的状态下，逐条去真做一遍：终止实例、拒绝给它 collateral、从外部观察请求的时序与大小、把 GPU 功耗与负载做关联、在它脚下滚动更换平台镜像。每一条都记录：你是否检测到了，以及客户端是否本可以检测到。

产出是一份你亲自验证过的残余风险清单，而不是从威胁模型模板里继承来的。模型提供方的安全团队真正会向你索要的，就是这份清单。

### 第 9 步 —— 清理

删掉实例与节点池、存储桶内容、KMS 密钥版本，以及负载均衡器。把冷启动预算表和前缀缓存那张分布图留着——这两份产物才是这个实验值得跑一趟的原因。
